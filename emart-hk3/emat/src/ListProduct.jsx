...
// import Header from 'e:/emart/emart-hk3/emat/src/header';
...
function ListProduct() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:3004/product/category/${categoryId}`);
  ...
  useEffect(() => {
    const fetchCategoryName = async () => {
      try {
        const response = await axios.get(`http://localhost:3004/category/${categoryId}`);
        setCategoryName(response.data.name);  // Lấy tên danh mục từ response
      } catch (error) {

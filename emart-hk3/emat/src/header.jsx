...
function Header({ username }) {
    ...
    const fetchCategories = async () => {
        try {
          const response = await axios.get('http://localhost:3004/category/xem');
          setCategories(response.data);
          console.log(response.data); // Assuming response data contains the list of categories
        } catch (error) {
    ...
    return (
        <header>
            <div className="bg-amber-400 h-14 w-full flex justify-center ">
                <Link to='/home'>
                    <img src="emart.png" alt="Grab" className=' ml-32 w-36 h-8 mt-2' />
                </Link>
              
                <div className="relative group">
                    <div className='flex flex-row text-white mt-4 ml-4 cursor-pointer' onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)}>
                        <p><FontAwesomeIcon icon={faBars} /></p>
                        <p className='text-lg ml-1'>Tất cả danh mục</p>
                    </div>
                    {isCategoryDropdownOpen && (
                        <div className="absolute bg-white shadow-lg rounded mt-2 w-80 -left-40 z-50">
                            {categories.map((category, index) => (
                                <div key={index} className="p-2 hover:bg-gray-100 relative group">
                                    <Link to={`/listProduct/${category._id}`} className='hover:text-orange-500 hover:cursor-pointer'>
                                        {category.name}
                                    </Link>
                                </div>
                            ))}
